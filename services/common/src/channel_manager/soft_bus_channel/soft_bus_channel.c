...
static int32_t OpenSoftBusChannel(const char *connectParams, int64_t requestId, int64_t *returnChannelId)
{
    ...
    if (memcpy_s(addr.info.mixAddr.addr, MIX_ADDR_MAX_LEN, connectParams, HcStrlen(connectParams) + 1) != EOK) {
        LOGE("Failed to copy connectParams!");
...
{
    LOGD("[SoftBus][In]: JoinLNN!");
    // (void)JoinLNN(channelId);
    LOGD("[SoftBus][Out]: JoinLNN!");
}

static int32_t GetLocalConnectInfo(char *jsonAddrInfo, int32_t bufLen)
{
    LOGD("[SoftBus][In]: GetLocalConnectionInfo!");
    // int32_t res = GetLocalConnectionInfo(jsonAddrInfo, bufLen);
    (void)jsonAddrInfo;
    (void)bufLen;
    LOGD("[SoftBus][Out]: GetLocalConnectionInfo!");
    // return res;
    return HC_SUCCESS;
}
...
SoftBus g_softBus = {
    ...
    .getLocalConnectInfo = GetLocalConnectInfo
};
